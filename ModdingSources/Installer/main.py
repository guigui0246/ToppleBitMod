...
def main():
    ...
    if (not options.setting_save_path and os.path.exists(os.path.join(user_config_dir("ToppleBitModding"), "config"))):
        alread_installed = True
    ...
    if any([mod not in AVAILABLE_MODS for mod in options.mod_list]):
        if not options.no_window:
            messagebox.showerror("Error", "One or more specified mods are not available.")
    ...
    if not options.setting_save_path:
        options.setting_save_path = os.path.join(user_config_dir("ToppleBitModding", ensure_exists=True), "settings.yaml")

    if not options.installer_install_path:
        options.installer_install_path = user_config_dir("ToppleBitModding", ensure_exists=True)
    ...
    if not options.no_window:
        ...
        installer_path_entry = tk.Entry(frame)
        installer_path_entry.pack(anchor=tk.W, pady=(0, 10), expand=True, fill=tk.X)
        if options.installer_install_path:
            installer_path_entry.insert(0, options.installer_install_path)

        # Settings path
        tk.Label(frame, text="Settings Path:*").pack(anchor=tk.W)
        settings_path_entry = tk.Entry(frame)
        settings_path_entry.pack(anchor=tk.W, pady=(0, 10), expand=True, fill=tk.X)
        if options.setting_save_path:
            settings_path_entry.insert(0, options.setting_save_path)

        # Game install path
        tk.Label(frame, text="Game Install Path:*").pack(anchor=tk.W)
        game_path_entry = tk.Entry(frame)
        game_path_entry.pack(anchor=tk.W, pady=(0, 10), expand=True, fill=tk.X)
        ...
        tk.Label(mod_list_frame, text="Mods to Install (comma-separated):").pack(anchor=tk.W)
        mod_list_scrollbar = tk.Scrollbar(mod_list_frame, orient=tk.VERTICAL)
        mod_list_box = tk.Listbox(mod_list_frame, selectmode=tk.MULTIPLE,
                                  yscrollcommand=mod_list_scrollbar.set, height=min(5, len(AVAILABLE_MODS)))
        ...
        tk.Checkbutton(frame, text="Backup before installing mods", variable=backup_var).pack(anchor=tk.W)

        restore_backup_var = tk.BooleanVar(value=options.restore_backup_on_failure)
        tk.Checkbutton(frame, text="Restore backup on failure", variable=restore_backup_var).pack(anchor=tk.W)

        start_on_startup_var = tk.BooleanVar(value=options.start_on_startup)
        tk.Checkbutton(frame, text="Start launcher on system startup", variable=start_on_startup_var).pack(anchor=tk.W)

        auto_run_var = tk.BooleanVar(value=options.auto_run)
        tk.Checkbutton(frame, text="Auto-run game after install", variable=auto_run_var).pack(anchor=tk.W)

        auto_update_installer_var = tk.BooleanVar(value=options.auto_update_installer)
        tk.Checkbutton(frame, text="Auto-update installer", variable=auto_update_installer_var).pack(anchor=tk.W)

        auto_update_game_var = tk.BooleanVar(value=options.auto_update_game)
        tk.Checkbutton(frame, text="Auto-update game", variable=auto_update_game_var).pack(anchor=tk.W)

        auto_update_var = tk.BooleanVar(value=options.auto_update_mods)
        tk.Checkbutton(frame, text="Auto-update mods", variable=auto_update_var).pack(anchor=tk.W)
        ...
        def on_submit():
            ...
            options.mod_list = [mod_list_box.get(i) for i in cast(Callable[[], list[int]], mod_list_box.curselection)()]
    ...
    shutil.copyfile(
        sys.argv[0],
        os.path.join(
            options.installer_install_path, os.path.basename(sys.argv[0])
        ),
    )
    with open(
        os.path.join(
            options.installer_install_path, "config"
        ),
        "w",
    ) as f:
